...
'this sub will count the amount of certain negative words existing within the reviews
Sub NegWord()
    ...
    'setting up line counter to display results
    Dim line As Integer
    line = 2
    ...
    'display results, set new line for next review
...
'this sub will count the number of certain positive words within the reviews
Sub PosWord()
    ...
    For Each strText In text
        ...
        'seting up comparing words placeholder and a counter
        Dim word As Variant
...
Sub LongestWordLength()
    ...
    'line number set to display results
    Dim line As Integer
    line = 2
...
Sub UCaseWordCount()
    ...
    'importing review text, preparing array to split
    Dim text As Range
    Set text = Range("A2:A1001")

    Dim words() As String
    
    'line number to set to display results
    Dim line As Integer
    line = 2

    'preparing variable to hold review text
    Dim strText As Variant
    
    'for loop goes through all of the reviews
    For Each strText In text

        'split text into variable, set counter to measure how many capital letters
        words = Split(strText, " ")
        Dim capitalCount As Integer
        capitalCount = 0
        
        'new placeholder for each word in review
        Dim word As Variant
            
            'goes through each word, and counts the amount of capitals in each word
            For Each word In words
...
Sub MeanWordLength()
    ...
    'for loop to go through each review
    For Each strText In text
        ...
        'set word to measure length and a counter for each word in the review
        Dim word As Variant
    ...
    'display calculated mean, set new line for next review
...
Sub EndingPunctuation()
    ...
    For Each strText In text
        ...
        'if text ends in "." , then set output value to 1
        If strText.Value Like "*[\.]" Then
            Range("I" & line).Value = 1
            line = line + 1
            
        'if text ends in "!", then set output value to 2
        ElseIf strText.Value Like "*!" Then
            Range("I" & line).Value = 2
            line = line + 1
            
        'if text ends in anything else, output value is 0
        Else
...
Sub ConsonantsVowelsRatio()
    ...
    'preparing variable to go through the reviews
    Dim strText As Variant
    ...
    For Each strText In text
        ...
        'set value to consonant divided by vowel, and go to next review
...
Sub FoodMentions()
    ...
    For Each strText In text
        ...
        'split review words into array
        words = Split(strText, " ")
        
        'set comparison placeholders and a counter
        Dim word As Variant
        ...
        foodWordTotal = 0
            ...
            'second for loop to go through each word, third nested loop to go through each food value
            For Each word In words
